...
fn main() {
    ...
    for code in 32..=126 {
        ...
        let (paths, dim) = vectomancy_text::parser::extract_text_outlines(&c.to_string(), font_bytes, size, 0.0).unwrap();
            ...
            for seg in p.data {
                match seg {
                    BezierSegment::MoveTo(pt) => {
                        current_pt = pt;
                        commands.push(Command { cmd_type: "M".to_string(), x: Some(pt.x), y: Some(pt.y), x1: None, y1: None, x2: None, y2: None });
                    },
                    BezierSegment::LineTo(pt) => {
                        current_pt = pt;
                        commands.push(Command { cmd_type: "L".to_string(), x: Some(pt.x), y: Some(pt.y), x1: None, y1: None, x2: None, y2: None });
                    },
                    BezierSegment::QuadraticTo(pt1, pt2) => {
                        ...
                        commands.push(Command { 
                            ...
                            x: Some(pt2.x), y: Some(pt2.y), 
                            x1: Some(cp1x), y1: Some(cp1y), 
                            x2: Some(cp2x), y2: Some(cp2y) 
                        });
                    },
                    BezierSegment::CubicTo(pt1, pt2, pt3) => {
                        current_pt = pt3;
                        commands.push(Command { cmd_type: "C".to_string(), x: Some(pt3.x), y: Some(pt3.y), x1: Some(pt1.x), y1: Some(pt1.y), x2: Some(pt2.x), y2: Some(pt2.y) });
                    },
                    BezierSegment::Close => {
                        commands.push(Command { cmd_type: "Z".to_string(), x: None, y: None, x1: None, y1: None, x2: None, y2: None });
                    },
                }
            }
            ast_paths.push(PathAST { color: "#fff".to_string(), commands });
        }
        
        atlas.insert(c.to_string(), GlyphInfo {
            width: dim.0 as f64,
            base_size: size as f64,
            ast: GlyphAST { ast_type: "Path".to_string(), paths: ast_paths }
        });
    }
    
    let json = serde_json::to_string(&atlas).unwrap();
    let mut file = File::create("/mnt/data/Workspace/Projects/xuepoo/xuepoo-www/static/ast/font_glyph_map.json").unwrap();
