...
- **Release Pipeline: Stale `vectomancy-text`/`vectomancy-video`/`vectomancy-cli` Versions**: The `v7.1.2` release tag published `vectomancy` and its six split geometry/transform/raster/svg/export/pipeline crates successfully, but `vectomancy-text` and `vectomancy-video` had not been bumped past their pre-decoupling `7.0.0`, so `cargo publish` for `vectomancy-text` failed with "crate already exists" partway through the job (`vectomancy-video`/`vectomancy-cli` never even got attempted). All ten workspace manifests (root, the six `crates/vectomancy-*`, `text/`, `video/`, `cli/`, plus their internal path-dependency `version` fields) are now bumped to `7.1.3` in lockstep.
- **`vectomancy-raster` Skeleton Extraction Quality**: `decode_raster_memory` previously used a simplified greedy neighbor walk that stopped at the first branch point, producing fragmented, noisy skeleton paths — different (and lower-quality) output than the facade's own `src/parser/raster.rs`, which does full Zhang-Suen thinning with endpoint/loop-aware path tracing. `vectomancy-raster` now uses the same Zhang-Suen thinning + endpoint-driven graph traversal as the facade, closing this gap (see `AGENTS.md`'s "Known Architecture Debt"). Added 5 tests covering closed loops, blank/uniform images, color sampling, and degenerate 1x1/2x2 inputs (previously untested edge cases).
- **Release Workflow: Fail Fast on Manifest/Tag Version Mismatch**: `.github/workflows/release.yml`'s `publish-crates` job now checks that every manifest's `version` matches the release tag _before_ publishing anything, rather than discovering a stale version halfway through (which is exactly what happened above — six crates got published to `7.1.2` before the mismatch surfaced, leaving the release half-done with no easy rollback).
